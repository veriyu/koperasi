        ...
        <div class=" col-xs-12">
          ...
          <table class="table table-bordered table-striped">
            ...
            <tbody>
              ...
              @else
                <?php
                  $SaldoBungaPinjaman = 0
                ?>
                @foreach($DataLaporan as $laporan)
                  @if($laporan->nilai_d != NULL && $laporan->no_akun == 600 || $laporan->nilai_k != NULL && $laporan->no_akun == 600)
                    <?php 
                      $SaldoBungaPinjaman = $SaldoBungaPinjaman + $laporan->nilai_d - $laporan->nilai_k;
                    ?>
                    <tr>
                      ...
                      @if($SaldoBungaPinjaman > 0)
                        <td class="text-right">{{ number_format($SaldoBungaPinjaman) }}</td>
                        <td class="text-right">0</td>
                      @else
                        <td class="text-right">0</td>
                        <td class="text-right">{{ number_format(abs($SaldoBungaPinjaman)) }}</td>
                      @endif
                    </tr>
                  @endif
                @endforeach
